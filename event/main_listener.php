	...
	public function viewtopic_body_postrow_add_karma_score_and_controls($event)
	{
		if ($event['row']['user_id'] != ANONYMOUS)
		{
			...
			if ($event['row']['user_id'] != $this->user->data['user_id'])
			{
				...
				$post_row['U_GIVEKARMA_POSITIVE'] = $this->helper->route("karma_givekarma_controller", array('karma_type_name' => 'post', 'item_id' => $event['row']['post_id']));
				$post_row['U_GIVEKARMA_NEGATIVE'] = $this->helper->route("karma_givekarma_controller", array('karma_type_name' => 'post', 'item_id' => $event['row']['post_id']));
